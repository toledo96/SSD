            ...
            if((y+dimension)-1 < posicion_y and (x + dimension)-1 < posicion_x and x_dimension < dimension and y_dimension < dimension):
                for y2 in range(y, (y+dimension)):
                    for x2 in range(x, (x+dimension)):
                        resta = pow(
                            recorte_matriz_temporal[y2][x2] - seccion_centro[y_dimension][x_dimension], 2)
                        sumatoria += resta
                        x_dimension += 1
                    y_dimension += 1
                    x_dimension = 0
            matriz_ssd[int((y+dimension)/2) +
                       1][int((x + dimension)/2) + 1] = int(sumatoria)
            sumatoria = 0
...
def accion_boton_escala_grises():
    ...
    escala_grises_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    # convierte una imagen a escala de grises
    escala_grises_image2 = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)
...
Button(ventana, text="Escala de grises",
       command=accion_boton_escala_grises).place(x=150, y=70)
Button(ventana, text="Imagen a color",
       command=accion_boton_rgb).place(x=250, y=70)
